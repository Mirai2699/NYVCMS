	...
	<?php
		...
		if(isset($_POST['btn_login'])){
			...
			if ($username == $userfromdb && $pass == $passfromdb)
			{
				...
				session_start();
	                 $_SESSION['Logged_In'] = $userfromdb;
	                 $_SESSION['UserRole'] = $typefromdb;
	                 //$_SESSION['email'] = $email;
	                
	                if($_SESSION['UserRole'] == "1")
	                {
	                  $header ='Location:Web/admin/views/dashboard.php?username='.$UserName.'';
	                  header($header);
	                }
	                else if($_SESSION['UserRole'] == "2")
	                {
	                  
	                  $header ='Location:Web/president/views/dashboard.php?username='.$UserName.'';
	                  header($header);
	                } 
	                else if($_SESSION['UserRole'] == "3")
	                {
	                  
	                  $header ='Location:Web/vice president/views/dashboard.php?username='.$UserName.'';
	                  header($header);
	                } 
	                else if($_SESSION['UserRole'] == "4")
	                {
	                  
	                  $header ='Location:Web/treasurer/views/dashboard.php?username='.$UserName.'';
	                  header($header);
	                } 
	                else if($_SESSION['UserRole'] == "5")
	                {
	                  
	                  $header ='Location:Web/sec-gen/views/attendance.php?username='.$UserName.'';
	                  header($header);
	                } 
